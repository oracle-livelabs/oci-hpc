...
Marketplace enthält Anwendungen und Images, die mit unserer Infrastruktur bereitgestellt werden können. Für Kunden, die Oracle Linux verwenden möchten, ist ein HPC-Clusternetzwerkimage verfügbar, das direkt auf dem Markt gestartet werden kann. Wir empfehlen, das [CFD Ready Cluster](https://cloudmarketplace.oracle.com/marketplace/en_US/listing/75645211) zu starten, das für CFD erforderliche Bibliotheken enthält.

1.  Wählen Sie im Marketplace oben rechts **App abrufen** aus.
    
2.  Wählen Sie die OCI-Region aus, und klicken Sie auf **Anmelden**.
    
3.  Prüfen Sie nach der Anmeldung die Version des HPC-Clusterimages, und wählen Sie dann das Compartment aus, in dem das Cluster gestartet wird. Akzeptieren Sie die Vertragsbedingungen, und starten Sie dann **Stack starten**.
    
4.  Geben Sie die restlichen Details des Stacks ein:
    
    *   Wählen Sie den gewünschten **AD** für die Compute-Ausprägungen und die Bastion aus.
    *   Kopieren - Fügen Sie den öffentlichen **ssh-Schlüssel** ein.
...
7.  Warten Sie, bis der Job "Erfolgreich" angezeigt wird, und navigieren Sie dann zu **Ausgaben**, um die privaten IPs der Bastion und Compute Nodes abzurufen.
...
5.  Wählen Sie im Hauptmenü **Compute, Instanzen** und dann "Instanz erstellen" aus.
    
6.  Ändern Sie das Image, und wählen Sie die Registerkarte **Oracle Image** aus. Wählen Sie **Oracle Linux 7 - HPC-Clusternetworkingimage** aus.
    
7.  Wählen Sie die **Availability-Domain** aus, in der Sie eine BM.HPC2.36-Instanz starten können
    
8.  Ändern Sie die **Ausprägung** unter "Bare Metal" und "Specialty" in "BM.HPC2.36".
...
11.  Wenn der Rechner hochgefahren ist, klicken Sie auf die erstellte Instanz. Wählen Sie unter **Weitere Aktionen** die Option **Instanzkonfiguration erstellen** aus. Sie können die Instanz jetzt unter **Weitere Aktionen** **beenden**.
    
12.  Wählen Sie im Hauptmenü **Compute** und dann **Clusternetzwerke** aus.
...
19.  Navigieren Sie in der Konsole zu **Compute**, **Instanzpools**, und erfassen Sie alle IP-Adressen für den Clusternetzwerkpool. Sie können diesen Befehl auch auf der Bastion verwenden, wenn in Ihrem privaten Subnetz nichts anderes ausgeführt wird.
